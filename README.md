...
[![Testing Nightly](https://github.com/Refinitiv/refinitiv-ui/actions/workflows/test_nightly.yml/badge.svg?branch=v6&event=schedule)](https://github.com/Refinitiv/refinitiv-ui/actions/workflows/browserstack.yml)
...
| `--browserstack` or `-bs` | You can run test on BrowserStack with specific browser(s). List of available browsers are following: `all`, `chrome`, `firefox`, `edge_previous`, and etc. <br>You can check the list of available browsers by add option `--help`. <br>This command option required BrowserStack API Key you must setting the three environment variables `BROWSERSTACK_USERNAME`, `BROWSERSTACK_ACCESS_KEY`, and `BROWSERSTACK_BUILD` <br>Usage: `npm run test button -- --browserstack chrome firefox_previous`
