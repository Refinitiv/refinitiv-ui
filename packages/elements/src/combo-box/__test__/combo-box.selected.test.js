...
describe('combo-box/Selected', () => {
  describe('Selection By Selected Property', () => {
    it('Data Selected: Afghanistan', async () => {
      ...
      expect(el.inputEl.value).to.equal('Afghanistan', 'Input is not reflected for AF');
      expect(el).shadowDom.to.equalSnapshot(snapshotIgnore);
      el.data = getData([3]);
      await elementUpdated(el);
      expect(el.value).to.equal('AL', 'Value is not reflected from selected');
      expect(el.inputEl.value).to.equal('Albania', 'Input is not reflected for AL');
      expect(el).shadowDom.to.equalSnapshot(snapshotIgnore);
      el.data = getData();
      await elementUpdated(el);
      expect(el.value).to.equal('', 'Value is not reset to empty string');
      expect(el.inputEl.value).to.equal('', 'Input is not reflected for ""');
      expect(el).shadowDom.to.equalSnapshot(snapshotIgnore);
    });
    it('Multiple. Data Selected: Afghanistan, Albania', async () => {
      ...
      expect(el.inputEl.value).to.equal('Afghanistan;  Albania', 'Input is not reflected for AF, AL');
      expect(el).shadowDom.to.equalSnapshot(snapshotIgnore);
      el.data = getData([2]);
      await elementUpdated(el);
      expect(String(el.values)).to.equal('AX', 'Values are not reflected from selected');
      expect(el.inputEl.value).to.equal('Aland Islands', 'Input is not reflected for AX');
      expect(el).shadowDom.to.equalSnapshot(snapshotIgnore);
      el.data = getData();
      await elementUpdated(el);
