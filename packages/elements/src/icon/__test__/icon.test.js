  ...
  describe('Should have correct result with CDN resource', () => {
    ...
    describe('Functional Tests', () => {
      ...
      it('should preload icons', async () => {
        ...
        const uniqueFirstValidIcon = generateUniqueName(iconName);
        const uniqueSecondValidIcon = generateUniqueName(iconName);
        const uniqueInvalidIcon = generateUniqueName(iconName);

        const uniqueFirstValidIconSrc = `${CDNPrefix}${uniqueFirstValidIcon}.svg`;
        const uniqueSecondValidIconSrc = `${CDNPrefix}${uniqueSecondValidIcon}.svg`;
        const uniqueInvalidIconSrc = `${CDNPrefix}${uniqueInvalidIcon}.svg`;

        createFakeResponse(tickSvg, responseConfigSuccess);
        let preloadedIcons = await Promise.all(
          preload(uniqueFirstValidIcon, uniqueSecondValidIcon)
        );
        createFakeResponse('', responseConfigError);
        ...
        expect(checkRequestedUrl(fetch.args, uniqueFirstValidIconSrc)).to.equal(true, 'should request first icon by name with CDN prefix');
        expect(checkRequestedUrl(fetch.args, uniqueSecondValidIconSrc)).to.equal(true, 'should request second icon by name with CDN prefix');
        ...
        el.setAttribute('icon', uniqueFirstValidIcon);
        await elementUpdated(el);
